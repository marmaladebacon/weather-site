...
const mapComponentState = {
  ...
  actions: {
    updatePosition: function(store:any, position:{lat:number, lng:number}) {
      store.commit('setPosition', position);
      return axios.get(`http://api.openweathermap.org/data/2.5/weather?lat=${position.lat}&lon=${position.lng}&units=metric&APPID=${APPID}`).then(data => {
        console.log(data);
      });
    }
  },
