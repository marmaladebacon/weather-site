...
jest.mock("../../src/utils/weatherAPI", () => {
  ...
  return {
    ...
    getFiveDayForecast(position: Position) {
      return new Promise((resolve, reject) => {
        resolve({ data: fiveDayJsonData }); // change to 5 day data
      });
    }
  };
});
...
describe("WeatherComponent", () => {
  ...
  test("check rendered", async () => {
    ...
    await wrapper.vm.updateCard({
      lat: 100,
      lng: 100
    });
    const weatherComponenentsArray = wrapper.findAll("weathercomponent-stub");
    // wrapper.html() returns the rendered html string, but here we can just find the count of the stubs
    // we expect 5 weatherComponents to be rendered after the card is updated.
    expect(weatherComponenentsArray.length).toEqual(5);
  });
});
